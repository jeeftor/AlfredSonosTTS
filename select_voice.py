...
def main(wf):
	...
	if stored_voice is not None:
	 	...
	 	if play:
	 		system('say -v %s Hello my name is %s!  This voice is currently selected.' % (selected_voice, selected_voice))
	 		wf.add_item('Current Selection: ' + stored_voice,'Select this option to use this voice, otherwise select a different voice below.', 
	 			icon=icon, 
	 			valid=True, 
	 			arg="config::exit")
	...
	for key,value in sorted(voices.items()):
		...
		wf.add_item(key + " " + flag + " [" + value + "]",'Select this voice', arg=key, valid=True, icon=get_gender(key))
